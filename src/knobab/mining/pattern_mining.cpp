...
//    std::vector<ActTable::record*> activationsCN, activationsCP;
    if (special_temporal_patterns) {
        ...
        if (alles_nexte) {
            ...
            auto a_beginend_copy = a_beginend;
...
//                    else activationsCP.emplace_back(a_beginend.first);
...
//                    else activationsCN.emplace_back(a_beginend.first);
            ...
            if (alles_prev) {
                ...
                auto& ref = declarative_clauses.emplace_back(clause,
                                                 result.support_generating_original_pattern,
                                                 (((double) (ntraces - alles_not_prev)) /
                                                  ((double) ntraces)),
                                                 -1);
//                ref.target_casusu = IsTargetPrev;
//                std::swap(ref.activations, activationsCP);
//                activationsCP.clear();
            }
            if (alles_next) {
                ...
                auto& ref = declarative_clauses.emplace_back(clause,
                                                 ...
                                                 (((double) (ntraces - alles_not_next)) /
                                                  ((double) ntraces)),
                                                 -1);
//                ref.target_casusu = IsTargetNext;
//                std::swap(ref.activations, activationsCN);
//                activationsCN.clear();
            }
        ...
        if (alles_precedence && alles_response) {
            ...
            auto& ref = declarative_clauses.emplace_back(clause,
                                             result.support_generating_original_pattern,
                                             (((double) (ntraces - std::max(alles_not_precedence,alles_not_response))) /
                                              ((double) ntraces)),
                                             -1);
        }
        if (alles_precedence) {
        ...
        if (alles_altresponse) {
            clause.casusu = "AltResponse";
            auto& ref = declarative_clauses.emplace_back(clause,
                                             result.support_generating_original_pattern,
                                             (((double) (ntraces - alles_not_altresponse)) /
                                              ((double) ntraces)),
                                             -1);
        }
        if (alles_response) {
...
/** B͟a͟l͟ì͟n͟ ͟d͟a͟ ͟S͟c͟i͟ò͟p͟p͟ algorithm **/
        ...
        if (x.first.size() == 1) {
            ...
            if (support == 1.0) {
                ...
                if (naif) {
                    ...
                    result.first.emplace_back(support, clause);
                } else {
                ...
                for (const auto& a : x) {
                    ...
                    for (const auto& b : y) {
                        ...
                        if (ratio.first >= minimum_support_threshold) {
                            ...
                            result.first.emplace_back(clause, support, local_support, -1);
...
//        std::cout << "   lift: " << counter.lift(lr) << " lift: " << counter.lift(rl) << std::endl;
        ...
        for (const auto& candidate: candidate_rule) {            // Generate the hypotheses containing a lift greater than one
            ...
            clause.n = 1;
            bool alsoFlip = false;
            if (candidate.clause.tail.empty()) {
                // CoExistence pattern
                A = candidate.clause.head.at(0);
                B = candidate.clause.head.at(1);
                clause.casusu = "CoExistence";
                alsoFlip = true;
            } else if (candidate.clause.tail.size() == 1) {
                A = candidate.clause.head.at(0);
                B = candidate.clause.tail.at(0);
                // <>A -> <>B
                clause.casusu = "RespExistence";
            };
            clause.left_act = kb.event_label_mapper.get(A);
            clause.right_act = kb.event_label_mapper.get(B);
            result.first.emplace_back(clause,
                                             candidate.support_generating_original_pattern,
                                             candidate.support_declarative_pattern,
                                             candidate.confidence_declarative_pattern);

            getAware(kb, special_temporal_patterns, only_precise_temporal_patterns, count_table,
                     minimum_support_threshold,
                     result.first, candidate, A, B, clause,
            ...
            if (alsoFlip) {
                ...
                getAware(kb, special_temporal_patterns, only_precise_temporal_patterns, count_table,
                         minimum_support_threshold,
                         result.first, candidate, B, A, clause,
                         negative_patterns ? &patterns_to_negate : nullptr);
            }
        }
    }
...
//    }
    ...
    if (init_end) {
        ...
        for (size_t act_id = 0; act_id < max_act_id; act_id++) {
            ...
            if (first_occ > minimum_support_threshold) {
                ...
                result.first.emplace_back(((double)(first_occ)) / ((double)log_size), clause);
            }
            if (last_occ > minimum_support_threshold) {
                clause.casusu = "End";
                clause.left_act = kb.event_label_mapper.get(act_id);
                result.first.emplace_back(((double)(last_occ)) / ((double)log_size), clause);
            }
        }
    ...
    result.second = ms_double.count();
    std::cout << result.second << "ms (mining)\n";
    return result;
}
