...
semantic_atom_set AtomizingPipeline::atom_decomposition(const std::string &act, bool isNegated) {
    ...
    return isNegated ? ordered_difference(atom_universe, S) : S;
}
...
double atomize_model(AtomizingPipeline &pipeline_data, std::vector<pattern_mining_result<DeclareDataAware>> &ref) {
    auto t1 = std::chrono::high_resolution_clock::now();
     {
        for ( auto& child : ref) {

            if (child.clause.dnf_left_map.empty() && (pipeline_data.strategy == AtomizeOnlyOnDataPredicates)) {
                pipeline_data.act_atoms.insert(child.clause.left_act);
                child.clause.left_decomposed_atoms.insert(child.clause.left_act);
            } else {
                ...
                if (!child.clause.dnf_left_map.empty()) {
                    ...
                    child.clause.left_decomposed_atoms = ordered_intersection(child.clause.left_decomposed_atoms,
                                                                         tmp);
                }
            }
            if (!child.clause.right_act.empty()) {
                if (child.clause.dnf_right_map.empty() && (pipeline_data.strategy == AtomizeOnlyOnDataPredicates)) {
                    pipeline_data.act_atoms.insert(child.clause.right_act);
                    child.clause.right_decomposed_atoms.insert(child.clause.right_act);
                } else {
                    child.clause.right_decomposed_atoms = pipeline_data.atom_decomposition(child.clause.right_act, false);
                    if (!child.clause.dnf_right_map.empty()) {
                        auto tmp = atomize_disjunction(pipeline_data, child.clause.dnf_right_map);
                        child.clause.right_decomposed_atoms = ordered_intersection(child.clause.right_decomposed_atoms,
                                                                              tmp);
                    }
                }
            }
...
double collect_data_from_declare_disjunctive_model(const yaucl::structures::any_to_uint_bimap<std::string>& map, AtomizingPipeline &pipeline_data, const std::vector<pattern_mining_result<DeclareDataAware>> &conjunction) {
    // old: pipeline_scratch, init_pipeline(3)
    //std::cout << "Collecting the atoms from the formula" << std::endl;
    auto t1 = std::chrono::high_resolution_clock::now();
    std::unordered_set<std::string> metAtomsGlobally;
    {
        for (const auto& declare_clause : conjunction) {
            ...
            pipeline_data.act_universe.insert(declare_clause.clause.left_act);
            if (!declare_clause.clause.right_act.empty())
                pipeline_data.act_universe.insert(declare_clause.clause.right_act);
            //DEBUG_ASSERT(declare_clause.conjunctive_map.empty());
