...
double atomize_model(AtomizingPipeline &pipeline_data, ConjunctiveModel &ref) {
    auto t1 = std::chrono::high_resolution_clock::now();
     {
        for ( auto& child : ref) {
...
double collect_data_from_declare_disjunctive_model(const yaucl::structures::any_to_uint_bimap<std::string>& map, AtomizingPipeline &pipeline_data, const ConjunctiveModel &conjunction) {
    // old: pipeline_scratch, init_pipeline(3)
    //std::cout << "Collecting the atoms from the formula" << std::endl;
    auto t1 = std::chrono::high_resolution_clock::now();
    std::unordered_set<std::string> metAtomsGlobally;
