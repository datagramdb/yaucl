...
void load_act_stream_to_knowledge_base(std::istream &stream, const std::string &filename, trace_visitor* tv) {
    std::string line;
    tv->enterLog(filename, filename);
...
//    trace_visitor* tv = (trace_visitor*)&output;
    switch (format) {
        ...
        case TAB_SEPARATED_EVENTS: // This never comes with data
            load_act_stream_to_knowledge_base(stream, filename, tv); break;
    }
}
