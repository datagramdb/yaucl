...
#include "yaucl/bpm/algos/atomization/AtomizingPipeline.h"
...
semantic_atom_set AtomizingPipeline::atom_decomposition(const std::string &act, bool isNegated) const {
    ...
    if (it == interval_map.end()) {
        ...
        for (size_t i = 0, N = max_ctam_iteration.at(act); i < N; i++) {
            cp.second = i;
            S.add(clause_to_atomization_map.at(cp));
        }
    }
...
std::ostream &operator<<(std::ostream &os, const AtomizingPipeline &pipeline) {
    ...
    for (const auto &k: pipeline.Mcal) {
        os << "   - " << k.first << " --> " << k.second.toString() << std::endl;
    }
    return os;
}
    ...
    for (const auto &ref: conj) {
        if (ref.second.casusu != TTRUE) {
            ...
            if (isFirst) {
                ...
                isFirst = false;
                if (S.cardinality() == 0) return S;
            } else {
                S &= pipeline_data.interval_decomposition(atom, false);
//                S = ordered_intersection(S, pipeline_data.interval_decomposition(atom, false));
                if (S.cardinality() == 0) return S;
            }
        }
...
double
atomize_model(AtomizingPipeline &pipeline_data, std::vector<pattern_mining_result<SimpleDeclareDataAware>> &ref) {
    auto t1 = std::chrono::high_resolution_clock::now();
    {
        for (auto &child: ref) {

            if (child.clause.dnf_left_map.empty() && (pipeline_data.strategy == AtomizeOnlyOnDataPredicates)) {
                auto left = pipeline_data.activity_label_to_id.get(child.clause.left_act);
                pipeline_data.act_atoms.add(left);
                child.clause.left_decomposed_atoms.add(left);
            } else {
                child.clause.left_decomposed_atoms = pipeline_data.atom_decomposition(child.clause.left_act, false);
                if (!child.clause.dnf_left_map.empty()) {
                    auto tmp = atomize_disjunction(pipeline_data, child.clause.dnf_left_map);
                    child.clause.left_decomposed_atoms = (child.clause.left_decomposed_atoms & tmp);
                }
            }
            if (!child.clause.right_act.empty()) {
                if (child.clause.dnf_right_map.empty() && (pipeline_data.strategy == AtomizeOnlyOnDataPredicates)) {
                    auto right = pipeline_data.activity_label_to_id.get(child.clause.right_act);
                    pipeline_data.act_atoms.add(right);
                    child.clause.right_decomposed_atoms.add(right);
                } else {
                    child.clause.right_decomposed_atoms = pipeline_data.atom_decomposition(child.clause.right_act,
                                                                                           false);
                    if (!child.clause.dnf_right_map.empty()) {
                        auto tmp = atomize_disjunction(pipeline_data, child.clause.dnf_right_map);
                        child.clause.right_decomposed_atoms = (child.clause.right_decomposed_atoms & tmp);
                    }
                }
            }
        }
...
// Assertion: one atom should only appear once!
            pipeline_data.atom_to_conjunctedPredicates[pipeline_data.clause_to_atomization_map.at(cp)] = k.second.at(i);
...
//                    child.left_decomposed_atoms = ordered_intersection(child.left_decomposed_atoms,
//                                                                       tmp);
//                }
//            }
...
//                        child.right_decomposed_atoms = ordered_intersection(child.right_decomposed_atoms,
//                                                                            tmp);
//                    }
//                }
//            }
...
//DEBUG_ASSERT(declare_clause.conjunctive_map.empty());
            for (const auto &itemList: declare_clause.clause.dnf_left_map) {
                for (const auto &item: itemList) {
                    ...
                    if (formula_numeric_atom.isStringPredicate()) {
                        auto V = std::get<0>(x);
                        pipeline_data.string_bulk_map[formula_numeric_atom.label][formula_numeric_atom.var].insert(
                                V.begin(), V.end());
                    } else {
                        auto V = std::get<1>(x);
                        pipeline_data.double_bulk_map[formula_numeric_atom.label][formula_numeric_atom.var].insert(
                                V.begin(), V.end());
                    }
                }
            }

            for (const auto &itemList: declare_clause.clause.dnf_right_map) {
                for (const auto &item: itemList) {
                    auto &formula_numeric_atom = item.second;
                    DEBUG_ASSERT(formula_numeric_atom.BiVariableConditions.empty());
                    DEBUG_ASSERT(formula_numeric_atom.varRHS.empty());
    ...
    for (size_t act_id = 0, N = pipeline_data.activity_label_to_id.int_to_T.size(); act_id<N; act_id++) {
        auto& act = pipeline_data.activity_label_to_id.int_to_T.at(act_id);
        auto it1 = pipeline_data.double_bulk_map.find(act);
        bool test1 = (it1 == pipeline_data.double_bulk_map.end()) || (it1->second.empty());
        auto it2 = pipeline_data.string_bulk_map.find(act);
        bool test2 = (it2 == pipeline_data.string_bulk_map.end()) || (it2->second.empty());
...
// init_pipeline(4)
//std::cout << "Collecting the atoms associated to no interval" << std::endl;
    for (const auto &act: pipeline_data.activity_label_to_id.int_to_T) {
...
// old: perform_insertion
        ...
        for (auto &ref: pipeline_data.interval_map) {
            ...
            ref.second = W;
            for (size_t i = 0, N = W.size(); i < N; i++) {
                atom_t FA = pipeline_data.generate_fresh_atom();
                pipeline_data.atom_universe.add(FA);
                pipeline_data.clause_to_atomization_map[std::make_pair(ref.first, i)] = FA;
                for (const DataPredicate &pred: W.at(i)) {
                    pipeline_data.Mcal[pred].add(FA);
                }
            }
            pipeline_data.max_ctam_iteration[ref.first] = W.size();
        }
