...
class DecisionTree {
    ...
    static float total_weights;
public:

    static float goodness;
...
template <typename T>
float DecisionTree<T>::goodness = std::numeric_limits<float>::min();

template <typename T>
float DecisionTree<T>::total_weights = std::numeric_limits<float>::min();
